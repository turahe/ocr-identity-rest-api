...
class read_image(object):
    ...
    def extract(self, extracted_result):
        ...
        for word in extracted_result.split("\n"):
            ...
            if "Nama" in word:
                word = word.split(':')
                self.result.nama = word[-1].replace('Nama ', '').replace('1', '')
                continue

            if "Tempat" in word:
                word = word.split(':')
                print(word, "tempat")
                self.result.tanggal_lahir = re.search(r"([0-9]{2}\-[0-9]{2}\-[0-9]{4})", word[-1])[0]
            ...
            if "Kecamatan" in word:
                self.result.kecamatan = word.split('—' if '—' in word else ':')[1].strip()
            if "Desa" in word:
                wrd = word.split()
                desa = []
                for wr in wrd:
